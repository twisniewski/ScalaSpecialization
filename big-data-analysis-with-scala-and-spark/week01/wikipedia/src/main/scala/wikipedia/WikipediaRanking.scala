...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("wiki-spark").setMaster("local")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (acc, article) => acc + Math.min(article.text.split(" ").count(_ == lang), 1),
    (acc1, acc2) => acc1 + acc2
  )
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(article => langs.filter(article.text.split(" ").contains(_)).map(lang => (lang, article)))
      .groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(article => langs.filter(article.text.split(" ").contains(_)).map(lang => (lang, 1)))
      .reduceByKey(_ + _)
      .sortBy {
        case (lang, count) => -count
      }.collect().toList
